...
class ConverterJobHandler(jobhandler.JobHandler):
    ...
    def post_processing(self, args):
        ...
        if not os.path.isfile(file_path):
            yield self.send_job_failed_notification("Cannot transfer file! File does not exist!")
            return

        file_size = os.path.getsize(file_path)
        with open(file_path, 'rb') as fp:
            file_hash = hashlib.sha256(fp.read()).hexdigest()
            yield self.send_job_finished_notification(file_size, file_hash)
            yield self.send_file(file_size, file_hash)
...
#     job.save()
    ...
    handler = ConverterJobHandler(Job.CategoryType.SCRATCH, job_ID, host, port, verbose, AUTH_KEY, ssl_options=ssl_ctx)
    handler.run(args)
